...
System.register("quest", [], function (exports_1, context_1) {
    "use strict";
    var __moduleName = context_1 && context_1.id;
    function Quest({ Web3, ethereum }) {
        ...
        class Env {
            ...
            async loadTypedefs(typedefs, force) {
                for (let { meta, type, defn } of Object.values(typedefs)) {
                    switch (meta) {
                        case 'enum':
                            new (this.Enum)(type, defn);
                            break;
                        case 'struct':
                            new (this.Struct)(type, defn);
                            break;
                        default:
            ...
            async fetchContractABI(contractNameOrAddress, network = DEFAULT_NETWORK, remember = true) {
                if (isAddressLike(contractNameOrAddress)) {
                    ...
                    if (instance && typeof instance.contractABI == 'object')
        ...
        class World {
            constructor(env = new Env) {
                ...
                this.loadedAccounts = this.loadAccounts();
                this.monkeyedWeb3 = this.monkeyWeb3();
            }
            ...
            async exec(steps, acc) {
                if (Array.isArray(steps)) {
                    ...
                    if (step instanceof Function) {
                        ...
                        try {
                            if (step.deploy)
                                return await this.deploy(step.deploy, step, acc);
                            else if (step.send)
                                return await this.send(step.send, step, acc);
                            else if (step.call)
                                return await this.call(step.call, step, acc);
                            else if (step.rpc)
                                return await this.rpc(step.rpc, step, acc);
                            else if (step.eval)
                                return await this.eval(step.eval, step, acc);
                        }
                        catch (e) {
            ...
            summarizeStep(step) {
                if (step.deploy)
                    return `{deploy: '${step.deploy}' as: '${step.as}' args: ${str(step.args)}}`;
                else if (step.send)
                    return `{send: '${step.send}' to: '${step.to}' args: ${str(step.args)}}`;
                else if (step.call)
                    return `{call: '${step.call}' on: '${step.on}' args: ${str(step.args)}}`;
                else if (step.rpc)
                    return `{rpc: '${step.rpc}' params: ${str(step.params)}}`;
                else if (step.eval)
                    return `{eval: '${step.eval}'}`;
        ...
        class ABI {
            ...
            decode(data, shape, strip = true) {
                if (shape instanceof Maybe)
                    try {
                        return this.decode(data, shape.just, strip);
                    }
                    catch (e) {
                        return;
                    }
            ...
            decodeOne(data, shape, strip = true) {
                if (shape instanceof Maybe)
                    try {
                        return this.decodeOne(data, shape.just, strip);
                    }
                    catch (e) {
                        return;
                    }
        ...
        const num = Object.assign(function num(numOrPromise, decimals = 1) {
        ...
        function EVMLib({ uint }) {
            ...
            return {
                ...
                $setTime: (date) => ({ rpc: 'evm_setTime', params: [date] }),
            ...
            const options = await world.rpc('evm_reset', { params: [
                    {
                        ...
                        gasLimit: await baseLastBlock.gasLimit,
                        gasPrice: '20000',
...
const __exp = __instantiate("quest", false);
